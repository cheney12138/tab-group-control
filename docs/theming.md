...
| 柔云团 + 大气光 | sky | `.search-plate` 背景 / `::after` | 天空要有"光"(右上柔光)与一层大尺度、边缘化开的云。**禁止等距小扇贝**:重复小弧线会读成简笔画描边(实测翻车后废弃) |
...
3. **颗粒纪律** —— 颗粒是签名,但只准在天空带加重(`.search-plate::before`,overlay `opacity 0.5`);文字所在云面的全局 `body::before` 只能是 multiply `opacity 0.09`。禁止用 `filter: blur()` / 发光替代颗粒。
