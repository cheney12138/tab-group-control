...
共享纹理资产(基座 `:root` 中的非度量项, 需在此登记): `--grain` —— 主题无关的中立灰胶片噪点, sky/ink 共用; 放基座单点定义, 删任一主题都不影响其他(不构成"白嫖")。铺法按底色亮度定: 中深底(sky 天空带)用 `background-blend-mode: overlay` 直接叠在天色上; 亮底(ink 纸)用 `mix-blend-mode: hard-light`(亮端增益≈1, 不压暗); pure multiply 只适合本就想要变深的场景。
  **强度上限看载体是否扁平**: 中深底叠加渐变(sky 天空带)可以到 std≈12, 因为明暗都落在中间调、读作质感; **扁平纯色亮面(sky 云面 / ink 纸面)上限只有 std≈3**, 超了就成污渍 —— 亮面上颗粒没有层次可融合, 深色侧的斑点直接被读成脏。
...
## 新增共享组件时的反向义务(每套主题都要出样式)

加一个新的共享 UI 组件(右键菜单、浮层、提示条……)时,**每套已落地主题都必须给出符合自身视觉语言的样式,禁止只留一套通用样式充当全部主题**。组件只准消费系统 token(`--pop-bg`/`--shadow-pop`/`--hairline`/`--text`…)拿到“能看”的默认态,但“像这个主题”必须由各主题自己的命名空间补足:

- linear: 发丝线 + 极淡浮层阴影
- ink: 宣纸底 + 墨线分隔
- sky: 云白面板 + 蓝调边 + 胶片颗粒(与云面同一套纹理纪律,不得用纯白/中性白)

通用样式不是“省事”,是把新组件排除在主题体系之外 —— 用户会立刻看出这块面板不属于当前主题。

其中**危险语义**必须走各主题自己的 `--danger` 系 token(如右键菜单里"关闭全部"那一项):硬编码一个红会在 ink 的宣纸上、sky 的云白上同时突兀 —— 红不只要"看得见",还要是**这套主题的红**。
